...
You can use parentheses, `()`, around conditions let us tell the database what to interpret first
...
```
 ...
 The LIKE operator works with a percent sign, `%`, representing the part of the field we don't really care about.

Find the state that the first chrachter matches a specific charachter (e.g., the letter `C`)
...
Match any state that ends with letter `N`:

```sqlite
SELECT first_name, last_name, state_Code FROM people WHERE state_code LIKE '%N';
```
...
Limt the number of results to a specific number 
...
The default ordering is the ascending order (`ASC`):
...
The `satate_code` will be ordered in ascending order, but the `last_name` will be ordered in descending order: 
...
* `LENGTH` : length of the cell (string),
...
**Cross JOIN**: the total number of rows is the number of row is table `people` multiplied by the number of rows in table `states`:
...
Same performace as `JOIN` with out using `JOIN`. It is better to explicitly use `JOIN`. 
...
You can use short (nick) name  for the table. (Personally, I don't like it)
...
* `SUBSTR`: sub-string (`substr(colon_name, index_start, index_end)`)
...
* `CAST`: change the type of the filed (e.g., form integer to charachter). Note that the result of the quiry below is sorted in alphanumbric order (i.e., 100 will come before 63)
...
The result of the quiry below will be 99 becuase it is highest in the alpha numeric order when it treatied as string. It is higher than 100! 
...
* `INSERT`: insert a value. For example, `INSERT INTO table (field) VALUES (value)`;
